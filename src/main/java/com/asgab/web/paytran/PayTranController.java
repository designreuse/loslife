...
import com.asgab.service.paytran.PayTranDetailService;
...
public class PayTranController {
  ...
  public String create(PayTranHeader payTranHeader, RedirectAttributes redirectAttributes, Model model, HttpServletRequest request,
    ...
    if (JCaptcha.validateResponse(request, request.getParameter("jcaptchaCode"))) {
      ...
      redirectAttributes.addFlashAttribute("message",
          CommonUtil.i18nStr(request, "创建交易信息成功, 交易编号:", "Transaction created and receipt uploaded successfully, Transaction Number:"));
      redirectAttributes.addFlashAttribute("message2", CommonUtil.i18nStr(request, "我们正在处理您的交易, 一经确认, 您将收到确认邮件。",
          "We are processing your request currently, you will receive a confirmation email soon."));
  ...
  public String toUpdate(@PathVariable("tranNum") Long tranNum, Model model, HttpServletRequest request) {
    ...
    model.addAttribute(model.addAttribute("action", "update"));
